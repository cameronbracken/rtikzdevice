...
\date{Version \Sexpr{tikzDevice:::getTikzDeviceVersion()}-{\bfseries\color{red}Beta}\qquad December, 2010}
...
\item[\code{tikzLatex}]{ Specifies the location of the \LaTeX{} compiler to be used by \pkg{tikzDevice}. Setting a default for this this option may help the package locate a missing compiler:

\begin{tikzCodeBlock}[title={Setting the default \LaTeX{} compiler in \code{.Rprofile}},listing style=sweavechunk]
...
\item[\code{tikzLatexPackages}]{ A character vector. These are the packages which are included when using the \code{standAlone} option as well as when font metrics are calculated.  If you use additional packages that affect fonts, such as mathpazo which provides a Palatino-style font, these packages should be added to this list.}
\begin{tikzCodeBlock}[title={Default},listing style=sweavechunk]
...
\item[\code{tikzMetricPackages}]{ A character vector. These are the extra packages which are additionally loaded when doing font metric calculations.  As you see below, the font encoding is set to Type 1. This is very important so that character codes of \LaTeX{} and \lang{R} match up.}
...
The overarching goal of the ‘tikzDevice’ is to provide seamless integration between text in R graphics and the text of LaTeX documents that contain those graphics.  In order to achieve this integration the device must translate font sizes specified in R to corresponding font sizes in LaTeX.  The issue is that font sizes in LaTeX are controlled by a ``base font size'' that is specified at the beginning of the document- typically 10pt.  There is no easy way in LaTeX to change the font size to a new numerical value, such as 16pt for a plot title.  Fortunately, the TikZ graphics system allows text to be resized using a scaling factor.  The \code{tikzDevice} calculates a scaling factor used to approximate other font sizes using the following three inputs:
...
\begin{itemize}
  ...
  \item{The ``character expansion factor'' parameter, specified using the ‘cex’ argument to functions such as described in the documentation of the \lang{R} function \code{par()}.}
  
  \item{The ``font size'' parameter, specified using the `ps’ argument to functions such as \code{par()} or the `fontsize’ argument to functions such as \code{gpar()}.}
...
Version 0.6.0 introduced support for (multibyte) Unicode characters in the text of graphics.  There are a few important implications/limitations of this feature:

\begin{description}

\item[System Requirements:]
A working version of XeLaTeX along with the packages fontspec and xunicode are required for direct processing of Unicode input.  If \code{tikzDevice} cannot find XeLaTeX then it will default to only allowing ASCII characters.

\item[Automatic Detection of Unicode Characters:]

\code{tikzDevice} will automatically recognize multibyte characters such that the metrics and string width for the character/string will be calculated using the \code{tikzUnicodeMetricPackages} and \code{tikzXelatexPackages} via a call to XeLaTeX (from the option \code{tikzXelatex}).  

\item[Unicode character and the \code{standAlone} option:]
There is no way to know before a tikz device is opened and the \code{standAlone} header is written whether any of the text input will have unicode characters. Consequently you must set the values of \code{tikzLatexPackages} manually before you open the device:
...
\begin{tikzCodeBlock}[listing style=sweavechunk]
tikz(..., packages=getOption('tikzXelatexPackages'))
\end{tikzCodeBlock}

\item[Encoding:] \code{tikzDevice} will try its best to convert characters from other encodings but do not count on it converting things correctly, best to do the conversion yourself beforehand to avoid unexpected output. 

\item[Fonts:] Having a Unicode character actually show up in your LaTeX document relies on the font you use having the glyph available. We leave it up to the user to know for themselves what is available.  Otherwise you will likely just get no output in place of where the character should be.

\item[plotmath:] There is specifically no support for input of plotmath characters as unicode since the user can simply input LaTeX math directly.  We strongly encourage the use of LaTeX math over plotmath for style and consistency's sake.  A consequence of this is that most of the \lang{R} examples and demos of plotmath won't work without significant manipulation (your milage may vary but you may get anything from errors to warnings to documents that mysteriously won't compile).  That is not to say that the output could not be duplicated with \code{tikzDevice} but the examples will not work out of the box. 

\item[Compiling:]
A graphic that contains UTF-8 characters must be compiled with XeLaTeX (or possibly LuaLaTeX) with the xunicode and fontspec packages enabled.

\item[ASCII only:]
Everything should be exactly the same as previous versions if only ASCII (single byte) characters are used (i.e. character codes less than 132).
...
\begin{tikzCodeBlock}[title={Example of a \TikZ{} environment including \code{bareBones} output},code body/.append style={codebody color=white},listing style=latexsource]
\begin{tikzpicture}
...
\subsubsection{Plotmath vs. tikzDevice output with mathematical output and UTF-8 Characters}

This exmple shows how UTF-8 characters and latex mathematics can be used to reproduce an example from \code{?text}.  Some characters might not show up in the vignette, see the source code for the complete example.  Figures \ref{fig:plotmathDefault} and \ref{fig:plotmathTikz} show the difference between \code{tikz()} output and the \code{pdf()} device. 

\begin{tikzCodeBlock}[title={Plotmath example replicated with tikzDevice},listing style=sweavechunk]
options(tikzLatexPackages=getOption('tikzXelatexPackages'))
options(tikzLatex='xelatex')
options(tikzSanitizeCharacters = c(getOption("tikzSanitizeCharacters"),"\\"))
options(tikzReplacementCharacters = c(getOption("tikzReplacementCharacters"),"\\textbackslash"))

tikz("plotmath.tex",width=7,height=7)
...
text(2, 1, "or Left/Bottom - JUSTIFIED at (2,1) by 'adj = c(0,0)'",
     adj = c(0,0))
text(4, 9, "$\\displaystyle\\hat{\\beta} = (X^tX)^{-1}X^ty$")
text(4, 8.4, 
    sanitizeTexString("\\displaystyle\\hat{\\beta} = (X^tX)^{-1}X^ty"),
    cex = .75)
text(4, 7, 
    "$\\displaystyle\\bar{x} = \\displaystyle\\sum_{i=1}^n\\frac{x_i}{n}$")
...
\begin{figure}[!h]
\centering
\includegraphics{img/plotmathDefault.pdf}
\caption{Default output with plotmath and the pdf device.}\label{fig:plotmathDefault}
\end{figure}

\begin{figure}[!h]
\centering
\includegraphics{img/plotmathTikz.pdf}
\caption{Plotmath output and UTF8 characters replicated with \code{tikz()} annotation.}\label{fig:plotmathTikz}
\end{figure}
...
\node[scale=4,text=black!40,inner sep=0pt,outer sep=0pt,below=0.25em of title] (char) {A};
\draw[color=black,very thick] (char.base west) -- (char.base east);
\draw[color=black,very thick] (char.north west) -- (char.north east);
\draw[color=red,very thick,<->,>=stealth] ([xshift=10pt]char.south west) -- ([xshift=10pt]char.north west);
...
Ascent is the distance between the baseline and the tallest point on a character's glyph. For the "A" printed to the left, the ascent has been calculated as: \the\yone
};
\end{tikzpicture}
...
\node[anchor = north west,outer sep=0pt,text width=0.6\textwidth] at ($(centerpoint) + (8em, 0)$){
Descent is the distance between the baseline and the lowest point on a character's glyph. For the "g" printed to the left, the descent has been calculated as: \the\yone 
};
\end{tikzpicture}
...
\node[scale=4,text=black!40,inner sep=0pt,outer sep=0pt,below= 0.25em of title.south] (char) {X};
\draw[color=black,very thick] (char.south west) -- (char.north west);
\draw[color=black,very thick] (char.south east) -- (char.north east);
\draw[color=red,very thick,<->,>=stealth] (char.mid west) -- (char.mid east); 
...
\node[anchor = north west,outer sep=0pt,text width=0.6\textwidth] at ($(centerpoint) + (8em, 0)$){
Width is the distance between the left and right sides of a character's glyph. For the "X" printed to the left, the width has been calculated as: \the\xone 
};
\end{tikzpicture}
...
Here we have started a standard \LaTeX{} input file by specifying \code{article} as the document class. We also add the \code{fontenc} package and specify \code{T1} as its option. This ensures we are using the Type 1 font encoding- by default \TeX and \LaTeX{} use an encoding called \code{OT1}. Why do we need to worry about font encodings? Well, a font encoding specifies which \code{ASCII} symbol codes map to which characters and by default, \lang{R} expects us to be using the Type 1 encoding (\lang{R} does support other encodings- but we're ignoring that for now). For example, in the Type 1 encoding, the character that corresponds to the \code{ASCII} code 60 is the less-than sign: `{\usefont{T1}{cmr}{m}{n}\char60}'. If we were to allow \TeX{} to retain its default \code{OT1} encoding, that same character code would instead map to an upside-down exclamation point: `{\char60}'.

The other two packages we load are the \code{tikz} package and its \code{calc} library. Essentially we will have \TikZ{} drop the character into a box and report some measurements concerning the size of that box. The last command, \code{batchmode} tells \LaTeX{} that there isn't any user available to interact with-- so it should bother to stop and ask any questions while processing this file.

The next step is to set up the part of the \LaTeX{} file that will actually calculate and report the widths we are looking for. As mentioned before, this is done by setting the character inside a \TikZ{} node and extracting the dimensions of the box that surrounds it. In an attempt to improve clarity, the following code will be presented as straight \LaTeX{} -- \code{getLatexCharMetrics} inserts it into the \code{texIn} file by means of \code{writeLines} as we have been doing all along. The string highlighted in {\color{red} red} should be replaced with the value of the variable charCode that was passed in to the function\code{getLatexCharMetrics}.
